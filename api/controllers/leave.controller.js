...
export const createLeave = async (req, res) => {
  try {
    ...
    const leave = await Leave.create({
      empId: employee._id, type, 
      startDate: new Date(startDate), endDate: new Date(endDate), reason, status: "PENDING",
    })
...
export const getLeaves = async (req, res) => {
  try {
    ...
    if(isAdmin) {
      ...
      const leaves = await Leave.find(where).populate('empId').sort({createdAt: -1});
